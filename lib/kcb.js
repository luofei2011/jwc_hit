...
function onRequest(req, res) {
    var lessons = {},  // 考试未结束的科目
        ...
        // 根据参数判断当前的周数和星期数
        opt = get_day(day),
        con_ = opt.is_even ? '单' : '双'; // 判断单双周

    // 下面的查询语句, 若该周是单周则略掉'双'周
    // 若该周为'双'周则略掉'单'周
    if (opt.is_today) {
        query = "SELECT * FROM kcb WHERE cid ='" + cid + 
                "' and day=" + opt.now + " and s_week<=" + opt.nowWeek + " and e_week >=" + opt.nowWeek + " and other != '" + con_ + "' order by time";
    } else {
        query = "SELECT * FROM kcb WHERE cid ='" + cid + 
                "' and s_week<=" + opt.nowWeek + " and e_week >=" + opt.nowWeek + " and other != '" + con_ + "' order by time";
    }
            ...
            jsdom.env(arr, function(err, win) {
                ...
                for ( i = 1; i < 6; i++ ) {
                    ...
                    for ( j = 1; j < 8; j++ ) { // 一星期七天
                        ...
                        for ( var item = 0; item < oSpans.length; item++ ) { // 循环处理每节课信息
                            ...
                            if (item % 3 == 2) {
                                if (spanHTML.indexOf('(') != -1) {
                                    is_contain = spanHTML.slice(spanHTML.indexOf('(') + 1, spanHTML.indexOf('(') + 2);
                                }
                                ...
                                lessArr.push(j);
                                // other 7
                                lessArr.push(is_contain);
                                // 今天的课程
                                // 先判断该课程是否已结束
                                if (parseInt(spanHTML[0]) <= nowWeek && parseInt(spanHTML[1]) >= nowWeek) {
                                    // 处理含有'单'和'双'周的情况
                                    if ((is_contain === '双' && opt.is_even) || // 若为'双'必须目标周是双周
                                            (is_contain == '单' && !opt.is_even) || // 若为'单'必须目标周为单周
                                                (is_contain === '')) { // 无单,双则代表正常上课
                                        // 若查询的是当天课表
                                        if (j == now && is_today) {
                ...
                for ( i = 0; i < kcb.length; i++ ) {
                    mysqlClient.query(
                        'INSERT INTO kcb ' +
                        'SET cid = ?, kc_name = ?, address = ?, s_week = ?, e_week = ?, time = ?, day = ?, other = ?',
                        kcb[i],    // {Array}
                        // 重复插入的情况, 这里未做处理, 都交由mysql自动检测
...
function get_day(week) {
    var week = week || "",
        is_today = true,  // 默认就显示今天的课程表
        is_even, // 判断单双周
        nowWeek, now;

    //is_today = (week === "") ? true : false;
    if (week === 'now_week' || week === 'next_week') 
        is_today = false;
...
function get_kcb(todayCourse, opt) {
    ...
    if (todayCourse.length) {
        ...
        if(opt.is_today) {
            ...
            for( ; i < len; i++) {
                todayCourse[i].length = 7; // 略掉other项, 保持数据一致性
                var day_time = todayCourse[i][6];  // 判断是星期几的课
    ...
    for ( ; i < len; i++ ) {
        for ( var item in arr_warp_obj[i] ) {
            ...
            if (arr_warp_obj[i].hasOwnProperty(item))
                // 为了保持数据的一致性, id和other不用处理
                if (item !== 'id' && item !== 'other')
